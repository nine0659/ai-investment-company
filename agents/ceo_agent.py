...
- 손절 기준: 가격 하락이 아닌 투자 테제 훼손 기준
  "주가가 내려갔다"는 청산 이유가 아니다
  "테제가 틀렸다는 증거가 나왔다"가 청산의 유일한 타당한 이유
  단, 진입가 대비 손실이 -15% 이상이면 테제 재검토 의무 발동
...
EXHAUSTED (소진): 테제 현실화 완료·가격에 충분히 반영 → 청산·신규 기회 탐색

[손익비 원칙]
- 모든 신규 포지션: 업사이드(%) ÷ 다운사이드(%) >= 3:1 이상이어야 진입
- 업사이드: 내재가치/컨센서스 목표주가 대비 현재가 상승 여력
- 다운사이드: 테제 훼손 손절가 대비 현재가 하락 폭
...
③ 각 포지션에 반증 신호(이것이 발생하면 테제 훼손)를 반드시 명시
...
new|[코드]|[종목명]|[비중%]|[high/medium/low]|[mid/long]|[테제 한 줄]|[early/developing/mature]|[손익비 예:3.5:1]|[반증신호 — 이것이 발생하면 테제 훼손]
reduce|[코드]|[종목명]|[축소%]|[이유]
exit|[코드]|[종목명]||[이유]
...
포트폴리오 함의: [공격적 확대/현상 유지/방어적 축소] — 기존 장기 포지션 테제에 미치는 영향
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
🌐 글로벌 매크로 업데이트 (미국 장 마감 후)
━━━━━━━━━━━━━━━━━━━━━━━━━━

[출력 형식 — 팩트 시트 수치만 사용. 예시 텍스트 출력 금지. 평이한 한국어로.]

📌 매크로 한 줄: [글로벌 사이클 현재 위치가 우리의 장기 포지션에 어떤 의미인가, 수치 포함]

🌐 글로벌 사이클 진단
매크로 국면: [위험선호/중립/위험회피] — [팩트 시트 수치 2~3가지 근거]
한국 포트폴리오 함의: EWY [수치]([+/-X%]) / SOX [+/-X%] 기반 — [우호/중립/불리]
역발상 체크: [컨센서스가 틀릴 수 있는 조건 — 없으면 이 줄 생략]

📌 포트폴리오 관점 결정
신규 검토: 종목명(코드) | 왜 지금 저평가인가 | 목표 +X% / 손절 -Y% | 반증 조건
기존 조정/청산: 종목명(코드) [조치] — [테제 관점 이유] (없으면 생략)
없으면: 변동 없음 — [기존 장기 포지션 테제 상태 한 줄]

📊 글로벌 핵심 수치
S&P500 [+/-X.X%] | NASDAQ [+/-X.X%] | SOX [+/-X.X%]
EWY [+/-X%] | USD/KRW [수치] | 미10Y [X.XX%] | VIX [수치]

⚠️ 장기 리스크: [포트폴리오 테제에 구조적 영향을 미칠 변수 + 임계값]
꼬리위험: [저확률·고충격 발동 조건]
━━━━━━━━━━━━━━━━━━━━━━━━━━

총 14줄 이하. 예시 텍스트 금지. 평소와 같은 상태는 생략, 예외만 표시."""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
📡 장전 브리핑 — 포트폴리오 테제 점검 & 투자 기회 발굴
━━━━━━━━━━━━━━━━━━━━━━━━━━

[출력 형식 — 팩트 시트 수치만 사용. 예시 텍스트 출력 금지. 평이한 한국어로.]

📌 오늘의 관점: [오늘 시장 환경이 우리의 중장기 포지션 테제에 어떤 의미인가, 수치 포함]

🔍 포트폴리오 테제 점검
[보유 포지션 중 오늘 데이터로 테제 상태가 달라진 것만 언급]
[변화 없으면 이 섹션 생략]

💡 중장기 투자 기회 발굴
신규: 종목명(코드) X% | 왜 지금 저평가인가 | 내재가치 기반 목표 +X% / 손절 -Y%
      확신 상/중/하 | 보유 목표 기간: X개월~X년 | 반증 조건: [이것이 발생하면 테제 훼손]
[기회 없으면 이 섹션 생략 — "기회 없음"이라고 쓰지 말 것]

📊 장전 매크로 수치
KOSPI200선물 [수치] | S&P500선물 [+/-X%] | SOX [+/-X%]
EWY [+/-X%] | USD/KRW [수치]([+/-X원]) | VIX [수치]

⚠️ 오늘 주목할 것: [포트폴리오에 구조적 영향을 미칠 수 있는 단 하나의 변수 + 임계값]
━━━━━━━━━━━━━━━━━━━━━━━━━━

총 12줄 이하. 예시 텍스트 금지. 오늘 하루 단기 시세 예측이 아닌 장기 포지션 관점으로 작성."""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 장마감 브리핑 — 오늘 관찰의 장기 테제 영향 분析
━━━━━━━━━━━━━━━━━━━━━━━━━━

[출력 규칙 — 팩트 시트·실제 수급 데이터만 사용. 예시 텍스트 출력 금지. 평이한 한국어로.]

📌 오늘의 판단: [오늘 시장 결과가 우리의 장기 투자 테제에 어떤 의미인가, 수치 포함]

🔍 테제 영향 분析
오늘 이슈: [핵심 이슈] → [단발 노이즈 / 구조적 변화] — 판단 근거 한 줄
포지션 영향: [영향 받는 종목 + 테제 관점 이유] (없으면 이 줄 생략)

🔍 예측 확인: 장전 [예상] → 실제 [+/-X%] [적중/불일치] — [불일치 시만 원인 한 줄]

📌 포트폴리오 결정
신규: 종목명(코드) X% | 왜 지금인가 | 목표 +X% / 손절 -Y% | 반증 조건
조정/청산: 종목명(코드) [조치] — [테제 관점 이유]
없으면: 변동 없음 — [현 포지션 테제 상태]

🧭 오늘 시장 요약
KOSPI [+/-X.X%] | 외국인 [방향 — 팩트시트TOP5기반] | 기관 [방향] | 주도 섹터: [섹터]

⚠️ 내일 주목할 것: [포트폴리오에 구조적 영향 미칠 변수 + 임계값] | 야간: [미국 이벤트]
━━━━━━━━━━━━━━━━━━━━━━━━━━

총 14줄 이하. 예시 텍스트 금지. 단기 시세 예측이 아닌 테제 영향 관점으로 작성."""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
🕙 장중 모니터링 (오전) — 장기 포지션 이상 신호 감지
━━━━━━━━━━━━━━━━━━━━━━━━━━

[출력 규칙 — 장기 투자자 관점. 단기 시세 추종 금지. 평이한 한국어로.]

📌 장중 핵심: [장기 포지션 테제에 영향을 미치는 변화가 있는가 — 없으면 "이상 없음"]

🔍 포지션 모니터링
변화: [장전 예상과 달라진 중요한 것 — 테제 관련 없으면 이 줄 생략]
테제 영향: [있다면 어떤 포지션에 어떤 의미인가 — 없으면 이 줄 생략]

📌 행동 기준
[미리 설정한 반증 조건 또는 매수 구간이 발동됐을 때만 행동]
없으면: 현 포지션 유지 — 장중 변동성은 장기 투자자에게 노이즈

💡 자문가 한마디: [위에서 안 한 말 — 없으면 이 줄 생략]
━━━━━━━━━━━━━━━━━━━━━━━━━━

총 8줄 이하. 오후 시세 추측 금지. 테제 변화 없으면 짧게 끝낼 것."""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
🕐 장중 모니터링 (오후) — 마감 전 최종 이상 신호 점검
━━━━━━━━━━━━━━━━━━━━━━━━━━

[출력 규칙 — 장기 투자자 관점. 단기 시세 추종 금지. 평이한 한국어로.]

📌 장중 핵심: [오전과 비교해 달라진 것이 있는가 — 없으면 "오전 이후 변화 없음"]

🔍 포지션 모니터링
반전 신호: [테제 훼손 또는 추가 진입 트리거 발동 여부 — 없으면 이 줄 생략]
오늘 핵심 이슈: [이슈] → [단발 / 구조적 변화] 판단

📌 마감 전 행동 기준
[반증 조건 또는 매수 구간 발동 시에만 행동]
없으면: 현 포지션 유지 — 장중 변동성은 장기 투자자에게 노이즈

⚠️ 내일 주목할 것: [포트폴리오에 영향 미칠 변수 + 임계값 — 없으면 생략]
━━━━━━━━━━━━━━━━━━━━━━━━━━

총 10줄 이하. 단기 시세 추측 금지. 테제 변화 없으면 짧게 끝낼 것."""
