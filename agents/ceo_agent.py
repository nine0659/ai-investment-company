...
[포지션 사이징 원칙 — 반드시 종목마다 명시]
- 확신도 상: 투자 가능 자금의 3~5% (매크로 RISK-ON 환경에서만)
- 확신도 중: 투자 가능 자금의 1~3%
- 확신도 하: 투자 가능 자금의 0.5~1%
- RISK-OFF 매크로 환경: 위 기준에서 50% 축소 적용
- 하루 전체 신규 포지션 합산 최대 10% (분산 원칙)

[매크로 레짐 적용 원칙]
- 매크로팀 레짐 보고를 먼저 확인하고 포지션 크기·섹터 방향에 반영
- RISK-ON: 성장주·반도체·방산 적극 공략 가능
- RISK-OFF: 방어주·배당주만, 신규 포지션 최소화
- NEUTRAL: 확신도 '상' 종목만, 소규모 진입

[출력 지침] 텔레그램으로 직접 전송되는 한국어 텍스트만 출력. 이모지 활용, 군더더기 없이.
이 지침 텍스트(출력 지침·형식 등의 메타 라벨)는 절대 출력에 포함하지 말 것."""
...
_STOCK_BLOCK_WITH_PRICE = """종목명(6자리코드)
   ...
   ├ 포지션 크기: 투자 가능 자금의 X% (확신도·매크로 레짐 기준 적용)
