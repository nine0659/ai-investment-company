...
투자자의 실제 보유 포지션과 관심종목 현황을 분석하여 구체적인 매매 전략을 제시하세요.

[분석 원칙]
- 보유 종목: 단순 보유가 아닌 "왜 지금 보유해야 하는가"를 재검증
- 수익 중인 종목: 추가매수 타당성 vs 익절 타이밍 판단
- 손실 중인 종목: 손절 vs 물타기 vs 홀드 — 반드시 명확한 근거 제시
- 워치리스트: 진입 조건 충족 종목은 즉시 진입 여부를 판단

[출력 규칙]
- 종목마다 행동 지시 명확화: 홀드/추가매수/분할매도/전량매도/진입
- 확률 수치 필수: "목표가 도달 확률 XX%" 형태
- 포지션 크기 명시: "현재 비중 X% → 목표 비중 Y%"
- 손절선 재확인: 현재 손절가가 적절한지 시장 상황 반영하여 재설정 여부 판단
- "상황을 지켜보자"는 표현 절대 금지 — 반드시 조건부 행동 지시
