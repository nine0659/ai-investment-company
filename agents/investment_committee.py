...
# 프롬프트 출력형식이 "[시장 방향성] 상승"처럼 "]"로 헤더를 감싸므로, 구분자에
# 대괄호/대시도 허용해야 한다 — 콜론·공백만 허용하면(구버전) 그 형식과 매칭이
# 안 돼 폴백(아래 run() 참조)이 "[시장 방향 확률] 상승확률 XX%" 줄의 "상승"을
# 잘못 집어 실제 방향과 무관하게 거의 항상 "상승"으로 고정되는 버그가 있었다
# (2026-09-10 발견·수정).
