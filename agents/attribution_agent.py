...
_SYSTEM = """당신은 포트폴리오 성과 분석가다.
바쁜 개인 투자자가 30초 안에 읽도록, 어려운 전문용어 없이 짧게 쓴다.

데이터 해석 규칙 (위반 금지):
- [현재 보유 포지션]의 수익률은 매수 이후 누적 수익률이다. 주간 수익률이 아니다.
  누적 보유수익률을 섹터 주간 등락률과 직접 비교하지 마라.
- 평가에 필요한 데이터가 없는 항목은 길게 추정하지 말고 "평가 불가 (데이터 없음)"
  한 줄로 끝내라.
- 수치는 제공된 데이터에 있는 것만 인용하라.

출력 형식 (전체 20줄 이내, 항목당 최대 2줄):

🏆 이번 주 점수: X/10
   매크로 X | 섹터 X | 종목 X | 타이밍 X | 투자관 X

① 시장 방향 판단: (1~2줄 — 맞았나 틀렸나, 왜)
② 섹터 선택: (1~2줄)
③ 종목 선택: (1~2줄)
④ 타이밍: (1~2줄)
⑤ 투자관 부합: (1~2줄)
...
def run_attribution():
    ...
    # 다중 트리거(스케줄러·수동·GH Actions) 대비 — 같은 날 1회만 발송
    from services.report_service import claim_report_slot, release_report_slot
    if not claim_report_slot(week_str, "attribution"):
        logger.info("[귀인분석] 오늘 이미 실행됨 — 스킵 (중복 발송 방지)")
        return
...
[현재 보유 포지션 — 수익률은 매수 이후 '누적' 수익률 (주간 수익률 아님)]
{portfolio_text}
...
위 데이터를 바탕으로 4가지 귀인 분석과 다음 주 개선 방향을 도출하라."""
    ...
    try:
        report = chat(_SYSTEM, context, max_tokens=900)
    except Exception as e:
        logger.error("[귀인분석] LLM 실패: %s", e)
        release_report_slot(week_str, "attribution")
