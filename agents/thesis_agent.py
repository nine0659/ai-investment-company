...
월간 투자 테제(Investment Thesis) 생성 에이전트
...
향후 6~12개월 한국 주식 시장을 바라보는 종합 투자 테제를 수립하라.

━━ [테제의 역할] ━━
이 문서는 향후 모든 일일 브리핑과 개별 종목 추천의 "헌법"이다.
개별 매매 신호는 반드시 이 테제의 방향성과 정합해야 한다.
테제에 반하는 단기 추천은 명시적 이유 없이 허용되지 않는다.
...
⑧ 테제 무효 조건 (이 중 하나라도 발생하면 테제 전면 재검토)
  - [조건 1]
  - [조건 2]
  - [조건 3]
...
_CEO_INJECT_SYSTEM = """아래 월간 투자 테제 전문에서
...
def run_thesis():
    """월간 투자 테제 생성, DB 저장, 텔레그램 발송."""
    now = datetime.now(_TZ)
    logger.info("[테제에이전트] 월간 투자 테제 수립 시작: %s", now.strftime("%Y-%m-%d"))
    ...
    try:
        ...
        market = {}
        logger.warning("[테제에이전트] 시장 데이터 실패: %s", e)
...
[최근 2주 귀인 분석 교훈 — 이번 테제에 반드시 반영할 것]
{recent_learnings if recent_learnings else '귀인 분석 이력 없음 (첫 번째 테제)'}
...
위 데이터를 바탕으로 향후 6~12개월 종합 투자 테제를 수립하라.
수치 근거 없는 판단 금지. 모든 방향 예측에 확률 명시."""
    ...
    try:
        ...
        logger.error("[테제에이전트] LLM 실패: %s", e)
        send_error_alert(f"월간 투자 테제 생성 실패: {e}")
        return
    ...
    try:
        ...
        logger.warning("[테제에이전트] CEO 요약 생성 실패: %s", e)
    ...
    try:
        ...
        logger.warning("[테제에이전트] DB 저장 실패: %s", e)

    # ── 텔레그램 발송 ────────────────────────────────────────────
    header = (
        f"📜 *월간 투자 테제* ({now.strftime('%Y년 %m월')})\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"경기 사이클·6~12개월 전망·자산배분·확신 아이디어\n\n"
    )
    send_message(header + full_report)
    logger.info("[테제에이전트] 완료")
