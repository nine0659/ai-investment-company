...
HISTORY = [
    {"date": "2026-06-29", "total_value": 110_000_000, "total_cost": 100_000_000,
     "total_pnl_pct": 10.0, "kospi_pct_ytd": 80.0, "nav_pct_ytd": 0.0,
     "alpha_ytd": 0.0, "kospi_close": 4000.0},
    {"date": "2026-07-03", "total_value": 112_000_000, "total_cost": 100_000_000,
     "total_pnl_pct": 12.0, "kospi_pct_ytd": 87.7, "nav_pct_ytd": 2.0,
     "alpha_ytd": 5.65, "kospi_close": 3854.0},
]
...
def test_alpha_uses_same_window_for_both_sides(stub_history):
    ...
    # 포트폴리오 배율 1.10→1.12 = +1.82%, KOSPI (3854-4000)/4000 = -3.65%
    # → 알파 +5.47%p (2026-07-10부터 pnl%p 차이가 아닌 배율 수익률)
    assert "+1.82%" in report
    assert "-3.65%" in report
    assert "+5.47%p" in report
...
def test_zero_kospi_close_no_crash(monkeypatch):
    ...
    report = nav_service.generate_nav_report(days=7)
    # KOSPI 결측 시 0을 종가로 쓰지 않고(−100% 오염 방지) 비교를 생략한다
